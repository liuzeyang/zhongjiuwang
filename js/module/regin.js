require(["../model/config"], function() {
	require(["jquery", "piccheck"], function($, piccheck) {
		...
		function mes() {
			var a = "f6474bf36700430983e7d98ac7182964";
			var b = "f05845422e4643d0ae1abef9023192aa";
			var c = getDate();
			var str = a + b + c
			var sig = str.MD5(32);
			$.ajax({
				type: "post",
				url: "/20150822/industryEmailSMS/sendEmailSMS?accountSid=f6474bf36700430983e7d98ac7182964&templateid=74951531&param={341232}" +
					"&to=" + $(".userid").val() + "&timestamp=" + c + "&sig=" + sig + "&respDataType=JSON",
				async: true,
		...
		$("#btn").click(function() {
			...
			if(count == 5) {
				$.ajax({
					type: "get",
					url: "/shopdata/userinfo.php?status=register&userID=" + $(".userid").val() + "&password=" + $(".password").val(),
					async: true,
					success: function(data) {
						if(data == "0") {
							alert("用户重名")
